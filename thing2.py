...
class PupThing():
    ...
    def loop_me(self,somevalue):
        if self.pupdict["toy"] == somevalue:
            # Self refers to any variable, function, object within the
            # same class
            status = True
        else:
            status = False
        # Return lets the function emit something back to the function which
        # called it
        return status

    def run(self):
        if self.loop_me("bone") and self.pupdict["bark"] == "borf":
            # self.loop_me is a function which returns a True|False status
            # based on the logic contained within it.
            # It accepts a parameter, which it sets as teh variable 'somevalue'
